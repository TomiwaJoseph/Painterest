...
def similar_fetch(request, pk, paint):
    ...
    similar_paintings =  all_similar_paintings[int(start_from):int(start_from) + int(each_load)]
    context = {
...
def category_fetch(request):
    ...
    if request.user.is_authenticated:
        ...
        if not all_slugs:
            painting_list = no_category_selected[int(start_from):int(start_from) + int(each_load)]
        else:
            painting_list = auth_all_painting[int(start_from):int(start_from) + int(each_load)]
    else:
        painting_list = no_category_selected[int(start_from):int(start_from) + int(each_load)]
...
def send_message(request):
    ...
    Message.objects.create(sender=request.user, recepient=painter, message=message, subject=subject)
    django_messages.success(request, "Message sent successfully.")
...
def add_comment(request):
    ...
    for user in all_followers:
        create_action(request.user, 'commented on', get_painting, user.user_from)
